...
You might still want to use a more speiciic grouping. You might be able to tweak your groups to make this happen.
...
The more cell types you have, the more agressive your FDR multiple hypothesis correction will need to be. Its best to remove or condense cell types that can never reach statistical significance. 
...
Calculate the transformed proportions that speckle needs.
```{r}
# If we didn't have fov pseudoreplicates
#props <- getTransformedProps(so$celltype_subset, so$individual_code, transform="logit")
...
props_order <- match(colnames(props$TransformedProps), celltype_summary_table$fov_name)
clusters    <- celltype_summary_table$cluster[props_order]
sample      <- celltype_summary_table$individual_code[props_order] 
condition   <- celltype_summary_table$condition[props_order]
