...
#source("code/subset_obj_seurat_v2.R") # no longer needed.
...
so$cluster_name <- factor(as.character(clustername_lookup[as.character(so$cluster)]), levels=clustername_lookup)
...
Not trying to get this annotation perfect, its just enough to demonstrate the methods.
...
```{r}
so.s <- subset(so, cells=so$cell[so$sample == 'sample2'])
