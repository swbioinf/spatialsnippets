...
Keep only those passed fov_clusters we saw earlier - this is what we'll use for calculating differential expression.  

We also need an annotation table to tell us what is in each fov_cluster (sample, celltype, condition...). Build this by filtering the celltype summary table made earlier. If we filter it to the same samples, in the same order as the pseudubulk matrix, it is much easier to build our differential expression model in the next step.
