...
NB: The LoadNanostring.X function workaround is in the [LoadNanostring.R script](https://github.com/swbioinf/spatialsnippets/blob/main/code/LoadNanostring.R) in the code directory of this repo. See [Seurat ticket](https://github.com/satijalab/seurat/discussions/9261). This issue may have been fixed.

```{r}
# Load LoadNanostring.X for keeping metadata e.t.c
source('code/LoadNanostring.R')
library(progressr) # used within
...
```{r}
# This dataset was originally loaded some time ago with a previous Seurat version,
# so its neccessary to update the object
so.raw <- UpdateSeuratObject(so.raw)
